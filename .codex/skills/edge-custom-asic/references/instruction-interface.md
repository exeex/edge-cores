...
Provide explicit accelerator-specific `set_*` commands, an asynchronous
`start`, and an await-like `sync`. Keep parameter mutation out of `start`.
...
Update every matching point together:
